...
public class VerticalPointer extends View {
    ...
    private void paintScale(Canvas canvas) {
        ...
        int totalDegree = mInitDegree + mDegree;
        ...
        for (int i = 0; i < scaleCount; i++) {
            ...
            if ((i - zeroIndex) % 3 == 0) {
                if (totalDegree - (5 * i) > 90) {
                    text = 2 * mInitDegree - (totalDegree - (5 * i)) + "";
                } else if (totalDegree - (5 * i) < -90) {
                    text = mInitDegree + (totalDegree - (5 * i)) + "";
                } else {
                    text = (totalDegree - (5 * i)) + "";
                }
                if (i == zeroIndex) {
                    text = "0";
        ...
        for (int i = 0; i < scaleCount; i++) {
            String text = "";
            if (i % 3 == 0) {
                if (mInitDegree + mDegree - (5 * i) > 90) {
                    text = 2 * mInitDegree - (mInitDegree + mDegree - (5 * i)) + "";
                } else if (mInitDegree + mDegree - (5 * i) < -90) {
                    text = mInitDegree + (mInitDegree + mDegree - (5 * i)) + "";
                } else {
                    text = (mInitDegree + mDegree - (5 * i)) + "";
                }
                if ((mInitDegree + mDegree - (5 * i)) == 0) {
                    mPaint.setColor(getContext().getResources().getColor(R.color.color_red_half));
                } else {
                    mPaint.setColor(mColorLong);
                }
